...
Automation lets the program and systems to scale 
...
[time_to_automate < (time_to_perform * amount_of_times_done)] <-- if this is the case, then it is the good candidate for automation, if no, then it is not

Pareto principle in IT = 20% of the system administration tasks that you perform are responsible for 80% of your work

Bit-rot = the process of software falling out of step with the environment

## Practical Example for Automation 

For example, checking the health of the computer

It can require to use a lot of checks, verifying that there is enough disk space, that the processor isn't overloaded, security updates, running services it is supposed to

We can us `shutil` module and `disk_usage` function to check the current available disk space 

About the cpu usage, we can use `psutil` module, this has the `cpu_percent` function that returns a number showing how much of the CPU is being used
...
print(du) # usage(total = 125487923200, used = 11327320064, free = 107742113792) <-- total number of bytes on disk, used and free

print(du.free/du.total*100) # 85.851249209 percentage of free disk space

psutil.cpu_percent(0.1) # 0.0 this function receives an interval of seconds and returns average percentage of usage in that interval, vary a lot with different calls 

```

Let's write two health checks script

```python
!/usr/bin/env python3 (# before ! mark)
...
Virtual Machine (VM) = computer simulated through software
